 ...
 * @method UsuarioQuery orderByBorrado($order = Criteria::ASC) Order by the borrado column
 *
 ...
 * @method UsuarioQuery groupByBorrado() Group by the borrado column
 *
 ...
 *
 * @method UsuarioQuery leftJoinToken($relationAlias = null) Adds a LEFT JOIN clause to the query using the Token relation
 * @method UsuarioQuery rightJoinToken($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Token relation
 * @method UsuarioQuery innerJoinToken($relationAlias = null) Adds a INNER JOIN clause to the query using the Token relation
 *
 * @method UsuarioQuery leftJoinMensajeNegocio($relationAlias = null) Adds a LEFT JOIN clause to the query using the MensajeNegocio relation
 * @method UsuarioQuery rightJoinMensajeNegocio($relationAlias = null) Adds a RIGHT JOIN clause to the query using the MensajeNegocio relation
 * @method UsuarioQuery innerJoinMensajeNegocio($relationAlias = null) Adds a INNER JOIN clause to the query using the MensajeNegocio relation
 *
 ...
 * @method Usuario findOneByBorrado(boolean $borrado) Return the first Usuario filtered by the borrado column
 *
 ...
 * @method array findByBorrado(boolean $borrado) Return Usuario objects filtered by the borrado column
 *
...
abstract class BaseUsuarioQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `ID`, `USUARIO`, `CLAVE`, `CORREO`, `CREATED_AT`, `UPDATED_AT`, `CREATED_BY`, `UPDATED_BY`, `ACTIVO`, `EMAIL`, `NUMERO_TELEFONO`, `PERFIL_ID`, `ADMINISTRADOR`, `NOMBRE_COMPLETO`, `BORRADO` FROM `usuario` WHERE `ID` = :p0';
        try {
    ...
    public function filterByBorrado($borrado = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(UsuarioPeer::BORRADO, $borrado, $comparison);
    }
    ...
    public function filterByToken($token, $comparison = null)
    {
        if ($token instanceof Token) {
            return $this
                ->addUsingAlias(UsuarioPeer::ID, $token->getUsuarioId(), $comparison);
    ...
    /**
     * Filter the query by a related MensajeNegocio object
     *
     * @param   MensajeNegocio|PropelObjectCollection $mensajeNegocio  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return   UsuarioQuery The current query, for fluid interface
     * @throws   PropelException - if the provided filter is invalid.
     */
    public function filterByMensajeNegocio($mensajeNegocio, $comparison = null)
    {
        if ($mensajeNegocio instanceof MensajeNegocio) {
            return $this
                ->addUsingAlias(UsuarioPeer::ID, $mensajeNegocio->getUsuarioId(), $comparison);
        } elseif ($mensajeNegocio instanceof PropelObjectCollection) {
            return $this
                ->useMensajeNegocioQuery()
                ->filterByPrimaryKeys($mensajeNegocio->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByMensajeNegocio() only accepts arguments of type MensajeNegocio or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the MensajeNegocio relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return UsuarioQuery The current query, for fluid interface
     */
    public function joinMensajeNegocio($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('MensajeNegocio');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'MensajeNegocio');
        }
    ...
    /**
     * Use the MensajeNegocio relation MensajeNegocio object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   MensajeNegocioQuery A secondary query class using the current class as primary query
     */
    public function useMensajeNegocioQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinMensajeNegocio($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'MensajeNegocio', 'MensajeNegocioQuery');
    }
